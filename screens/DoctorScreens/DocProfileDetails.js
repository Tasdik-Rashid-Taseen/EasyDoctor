...
const DocProfileDetails = ({ navigation, route }) => {
    ...
    const onPressSaveEdit = () => {
        ...
        onAuthStateChanged(authentication, async (user) => {
            if (user) {
                ...
                console.log("Update")
                await updateDoc(doc(db, 'doctorList', userID), {
                    doc_username: newUsername,     
                    doc_num: newNumber,
                })
            ...
            <View style={styles.container}>
                ...
                <View style={styles.profile_opts}>
                    <TouchableOpacity style={styles.profile_opt} >
                        ...
                        <Text style={{ fontSize: 18, }}>{doctor.doc_username}</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={styles.profile_opt}>
                        <Text style={{ fontSize: 18, }}>Gender</Text>
                        <Text style={{ fontSize: 18, }}>{doctor.doc_gender}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.profile_opt}>
                        <Text style={{ fontSize: 18, }}>Number</Text>
                        <Text style={{ fontSize: 18, }}>{doctor.doc_num}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.profile_opt}>
                        <Text style={{ fontSize: 18, }}>Mail</Text>
                        <Text style={{ fontSize: 18, }}>{doctor.doc_email}</Text>
                    </TouchableOpacity>
                    <Modal
                        animationType='fade'
                    ...
                    >
                        <View style={styles.modalView}>
                            ...
                            <TextInput style={styles.modalTextInput} onChangeText={(text) => {
                                setinputText(text)
                                setnewUsername(text)
                            }} defaultValue={doctor.doc_username} editable={true} multiline={false} maxLength={200}></TextInput>
                
                            <TextInput style={styles.modalTextInput} onChangeText={(text) => {
                                setinputText(text)
                                setnewNumber(text)
                            }} defaultValue={doctor.doc_num} editable={true} multiline={false} maxLength={200}></TextInput>
...
export default DocProfileDetails
